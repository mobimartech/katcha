...
export default function UserDetailScreen({
  ...
  const [followersCount, setFollowersCount] = useState(
    targetData?.followers || 0
  );
  const [followingCount, setFollowingCount] = useState(
    targetData?.following || 0
  );
    ...
    void (async () => {
      try {
        ...
        if (targetData?.result) {
          ...
          const facepile = (base.profile_context_facepile_users ||
            []) as FacepileUser[];
        ...
        setFollowersCount(
          followersResult.total || followersResult.followers?.length || 0
        );
        ...
        setFollowingCount(
          followingResult.total || followingResult.following?.length || 0
        );
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              console.log('[UserDetail][Delete] Request', {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.primary }}>
      ...
      >
        <View
          style={[styles.sheet, { backgroundColor: colors.surface, flex: 1 }]}
        >
          {/* Title row */}
          ...
          <View style={styles.profileRow}>
            ...
            <View style={{ marginLeft: 14, flex: 1 }}>
              <Text style={[styles.handleLarge, { color: colors.text }]}>
                @{username}
              </Text>
              <Text style={[styles.realName, { color: colors.text }]}>
                {displayName}
              </Text>
              <Text
                style={[styles.updatedText, { color: colors.textSecondary }]}
              >
                Updated 2h ago
              </Text>
            </View>
            ...
            <View style={styles.platformRow}>
              <View
                style={[
                  ...
                  {
                    backgroundColor:
                      platform === 'tiktok' ? '#000000' : colors.primary,
                  },
                ]}
              >
                <Icon
                  name={
                    platform === 'tiktok' ? 'logo-tiktok' : 'logo-instagram'
                  }
                  size={20}
                  color="#FFFFFF"
                />
              </View>
              <Text style={[styles.platformHandle, { color: colors.text }]}>
                @{username}
              </Text>
            </View>
            ...
            <View style={styles.metricsGrid}>
              <View style={styles.metricBox}>
                <Text style={[styles.metricValue, { color: colors.text }]}>
                  {formatCount(info?.follower_count || followersCount)}
                </Text>
                <Text
              ...
              <View style={styles.metricBox}>
                <Text style={[styles.metricValue, { color: colors.text }]}>
                  {formatCount(info?.following_count || followingCount)}
                </Text>
                <Text
                  style={[styles.metricLabel, { color: colors.textSecondary }]}
                >
              ...
              <View style={styles.metricBox}>
                <Text style={[styles.metricValue, { color: colors.text }]}>
                  {formatCount((targetData as any)?.video_likes || 0)}
                </Text>
                <Text
                  style={[styles.metricLabel, { color: colors.textSecondary }]}
                >
              ...
              <View style={styles.metricBox}>
                <Text style={[styles.metricValue, { color: colors.text }]}>
                  +{formatCount((targetData as any)?.new_fans_week || 0)}
                </Text>
                <Text
                  style={[styles.metricLabel, { color: colors.textSecondary }]}
                >
